...
//		dp.setProperties({'id':dp.getProperty('name'), 'readonly':true});
	...
	create: function(dp){
		...
		dp.container = new Element('div', {'class':'dp_container'});
		$(document.body).adopt( dp.container );		
		...
		/*
			EDIT 02/09/2009
			added setTimeout to fix calendar update bug in Safari
		*/
	...
	remove: function(dp){
		$clear(dp.interval);
		dp.active = false;
		if (dp.container) dp.container.destroy();
		dp.calendar = false;
		dp.container = false;
